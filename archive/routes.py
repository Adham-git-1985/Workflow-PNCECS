...
@login_required
def recycle_bin():
    q = ArchivedFile.query.filter(ArchivedFile.is_deleted.is_(True))
...
def purge_recycle_bin():
    """Permanently delete files that exceeded the retention period."""
    ...
    for f in candidates:
        # Never purge files linked to workflow requests
        ...
        try:
            ...
            # Ignore FS errors; still remove DB record
            pass
        ...
        db.session.add(
            AuditLog(
                ...
                target_type="ARCHIVE_FILE",
                target_id=f.id,
                note=f"File '{f.original_name}' permanently purged (retention {days} days)"
            )
        )
    ...
    if purged:
        flash(f"تم حذف {purged} ملف نهائيًا (تجاوزت {days} يوم)", "success")
    else:
        flash("لا توجد ملفات منتهية للحذف النهائي حاليًا.", "info")

    if skipped:
        flash(f"تم تجاوز {skipped} ملف لأنه مرتبط بطلب/مسار عمل.", "warning")
...
def purge_single_from_recycle_bin(file_id):
    """Permanently delete a single file from recycle bin.

    Allowed for ADMIN or the file owner.
    Safety rules:
      - Must be already soft-deleted (is_deleted=True)
      - Cannot purge files linked to workflow requests
      - Cannot purge signed files
    ...
    # Do not allow purging signed files
    if getattr(f, "is_signed", False):
        flash("لا يمكن حذف ملف موقّع نهائيًا.", "danger")
        return redirect(url_for("archive.recycle_bin"))
    ...
    db.session.add(
        AuditLog(
            action="ARCHIVE_PURGE_SINGLE",
            user_id=current_user.id,
            target_type="ARCHIVE_FILE",
            target_id=f.id,
            note=f"File '{f.original_name}' permanently purged manually"
        )
    )
