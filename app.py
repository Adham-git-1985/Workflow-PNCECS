...
def my_requests():
    ...
    if status:
        if status == "in_progress":
            ...
            base_query = base_query.filter(WorkflowRequest.status == status.upper())

    requests = base_query.order_by(WorkflowRequest.id.desc()).all()
...
def inbox():
    allowed_statuses = [
        s for s, r in STATUS_ROLE_MAP.items()
        if r == current_user.role
    ]
