...
def decide_step(req_id: int, step_order: int, actor_user_id: int, decision: str, note: str = "", auto_commit: bool = False):
    ...
    if decision == "REJECTED":
        ...
        follower_ids.discard(int(actor_user_id))
        follower_ids.discard(int(req.requester_id))
        if follower_ids:
            fmsg = f"تحديث على المسار: تم رفض الطلب #{req.id} (الخطوة {step_order}) من {actor_label}"
            if note:
    ...
    if not next_step:
        ...
        if follower_ids:
            _notify_users(
                sorted(follower_ids),
                message=f"تحديث على المسار: تمت الموافقة النهائية على الطلب #{req.id} من {actor_label}" + (f" | ملاحظة: {note}" if note else ""),
                ntype="WORKFLOW",
            )
    ...
    if follower_ids:
        _notify_users(
            sorted(follower_ids),
            message=f"تحديث على المسار: تمت الموافقة على الطلب #{req.id} (الخطوة {step_order}) وتحويله للخطوة {next_order} من {actor_label}" + (f" | ملاحظة: {note}" if note else ""),
            ntype="WORKFLOW",
        )
