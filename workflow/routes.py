...
def upload_attachment(request_id):
    ...
    attachment_batch_id = _new_attachment_batch_id()
    saved_paths = []
    try:
        for fs in files:
...
_AUDIT_ATTACHMENT_BATCH_RE = re.compile(
    r"(?:^|\|)\s*(?:attachment_batch|batch)\s*=\s*([A-Za-z0-9_-]+)",
    re.IGNORECASE,
)


def _new_attachment_batch_id() -> str:
    """Return an opaque identifier shared by all files in one upload action."""
    return uuid.uuid4().hex


def _attachment_audit_batch_id(note: str | None) -> str | None:
    """Read the optional batch marker from an attachment audit note."""
    match = _AUDIT_ATTACHMENT_BATCH_RE.search(str(note or ""))
    return match.group(1).strip() if match else None


def _attachment_activity_group_key(log: AuditLog):
    """Return a stable display-only key for one multi-file upload.

    New rows carry an explicit batch marker.  Older rows do not, so use the
    second-level timestamp as a backwards-compatible fallback; attachment
    rows generated by one request are written consecutively and share that
    timestamp in the existing data.
    """
    note = audit_display_note(log)
    batch_id = _attachment_audit_batch_id(note)
    action = str(getattr(log, "action", "") or "").strip().upper()
    actor_id = audit_display_actor_id(log) or getattr(log, "user_id", None)
    request_id = getattr(log, "request_id", None)
    if batch_id:
        return ("batch", request_id, actor_id, action, batch_id)

    created_at = getattr(log, "created_at", None)
    if created_at is None:
        return ("legacy", action, getattr(log, "id", None))
    try:
        # The fallback is intentionally limited to the second.  It merges the
        # historical bulk-upload rows while avoiding unrelated uploads made
        # minutes apart; future rows use the explicit marker above.
        created_at = created_at.replace(microsecond=0)
    except (AttributeError, TypeError, ValueError):
        pass
    return ("legacy", request_id, actor_id, action, created_at)


def _collapse_attachment_activity_entries(entries: list[dict]) -> list[dict]:
    """Combine attachment activity cards from one upload into one card."""
    collapsed: list[dict] = []
    grouped: dict[tuple, dict] = {}

    for entry in entries:
        if not entry.get("_is_attachment_activity"):
            collapsed.append(entry)
            continue

        key = entry.get("_attachment_group_key")
        current = grouped.get(key)
        if current is None:
            current = dict(entry)
            current["_attachment_names"] = []
            name = entry.get("_attachment_name") or ""
            if name:
                current["_attachment_names"].append(name)
            grouped[key] = current
        ...
        name = entry.get("_attachment_name") or ""
        if name:
            # The activity query is newest-first; prepend subsequent names so
            # the combined card follows the original upload order.
            current["_attachment_names"].insert(0, name)

    for entry in collapsed:
        names = entry.pop("_attachment_names", [])
        attachment_action = entry.pop("_attachment_action_code", "WORKFLOW_ATTACHMENT_UPLOADED")
        entry.pop("_is_attachment_activity", None)
        entry.pop("_attachment_group_key", None)
        entry.pop("_attachment_name", None)
        if len(names) > 1:
            if attachment_action == "WORKFLOW_ATTACHMENT_DELETED":
                entry["action"] = f"تم حذف {len(names)} مرفق/مرفقات من المسار"
            else:
                entry["action"] = f"تمت إضافة {len(names)} مرفق/مرفقات"
            entry["note"] = "أسماء المرفقات:\n" + "\n".join(
                f"• {name}" for name in names
            )
        elif names:
            entry["note"] = f"اسم المرفق: {names[0]}"
...
def view_request(request_id):
    ...
    for log in audit:
        ...
        attachment_activity = action == "WORKFLOW_ATTACHMENT_UPLOADED"
        attachment_note = _user_facing_audit_note(log, action, files_map)
        attachment_name = ""
