...
class CorrespondenceIntakeTests(unittest.TestCase):
    ...
    def test_workflow_form_exposes_attachment_analysis_and_field_dump(self):
        ...
        self.assertIn("templateSelect.addEventListener('change', renderPredefinedPreview)", template)
        self.assertIn('predefined_template_steps=predefined_template_steps', routes)
        self.assertIn('عرض تحليل وتتبع المسار', template)
        self.assertNotIn('aria-controls="dynamicPathPreviewPanel" disabled', template)
        self.assertIn('أضف وجهة أولاً لعرض المسار الفعلي.', template)
        self.assertIn("previewToggle.addEventListener('click', schedulePreview)", template)
        self.assertIn('ثم اضغط «إضافة الجهة نفسها إلى المسار»', template)
        ...
        self.assertIn('مسار إداري عمودي تلقائي', template)
        self.assertIn('منشئ الطلب — نقطة انطلاق المسار العمودي', template)
        self.assertIn('"origin": result.get("origin")', routes)
        self.assertNotIn('dynamic-route-start', template)
    ...
    def test_workflow_close_action_and_portal_return_button_layout_are_exposed(self):
        ...
        self.assertNotIn('list-group list-group-numbered mb-0', request_template)
        self.assertIn("org_node_approver_names_map.get", request_template)
