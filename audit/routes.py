...
def system_timeline_export_excel():
    """Export System Activity Timeline to Excel (respects filters).

    Notes:
      - Exported in newest-first order.
      - Default window: last 7 days (same as page).
      - You can control max rows via ?limit=... (default 20000).
    """
    action = request.args.get("action")
    user_id = request.args.get("user_id", type=int)
    date_from = request.args.get("date_from")
    date_to = request.args.get("date_to")
    days = request.args.get("days", type=int)
    limit = request.args.get("limit", type=int) or 20000

    q = _apply_message_visibility_filter(AuditLog.query.outerjoin(User))
    ...
    # Default window (same behavior as UI)
    if not date_from and not date_to and not days:
        days = 7
    ...
    for l in logs:
        rows.append([
            ...
            l.action,
            (l.user.email if getattr(l, "user", None) else "System"),
            getattr(l, "request_id", "") or "",
            getattr(l, "target_type", "") or "",
            getattr(l, "target_id", "") or "",
            getattr(l, "old_status", "") or "",
            getattr(l, "new_status", "") or "",
            getattr(l, "note", "") or "",
        ])

    data = make_xlsx_bytes("Timeline", headers, rows)
    filename = f"system_timeline_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.xlsx"
    return send_file(
        BytesIO(data),
